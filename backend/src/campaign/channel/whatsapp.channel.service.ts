...
export class WhatsappChannelService {
     ...
     public sendMessage =async (contact, message,  image, page):Promise<boolean>=>{
        let success = false;
        try {
          await page.locator("div#side button.-Jnba").click({ timeout: myTime.TIME_OUT }); //annuler la recherche
        } catch (e) {}
    ...
    try {
      await page.locator("#main header.AmmtE").click({ timeout: myTime.TIME_OUT });
    } catch (e) {
    ...
    try {
      ...
      let spanLocator 
      try {
        spanLocator = await page.waitForSelector("div._2Ts6i._1xFRo  span.enbbiyaj.e1gr2w1z.hp667wtd", { timeout: myTime.TIME_OUT });
      } catch(e) {}

      //Pour gérer le cas des comptes professionnels
      if(!spanLocator){
        spanLocator=await page.waitForSelector("div._2Ts6i._1xFRo  span._11JPr > span.fe5nidar.fs7pz031.tl2vja3b.e1gr2w1z", { timeout: myTime.TIME_OUT });
      }
    ...
    try {
      await page.locator("div._2Ts6i._1xFRo header.cm280p3y div.kk3akd72.svlsagor").click({ timeout: myTime.TIME_OUT }); 
    } catch (e) {}
    ...
    if (image != undefined) {
        ...
        await  sleep(myTime.TIME_WAIT_ACTION);  
        try {
          await page.locator("button.tvf2evcx.oq44ahr5.lb5m6g5c.svlsagor.p2rjqpw5.epia9gcq").first().click({ timeout: myTime.TIME_OUT });
          success = true;
        } catch (e) {
          //logger.error(contact, problem.select_button_send);
          return;
    ...
    public sendAllMessage = async (your_message, list_contact, image): Promise<void> => {
            ...
            fs.createReadStream(list_contact)
                ...
                .on("end", async () => {
                    ...
                    do {
                        ...
                        const messageTransformed = message.replace(new RegExp(balise_replace.PRENOM, 'g'), prenom);
                        const success = await this.sendMessage(phone, messageTransformed, image, page);
                    ...
                    messagesSentAllCount = messagesSentFailedCount + messagesSentSuccedCount;
